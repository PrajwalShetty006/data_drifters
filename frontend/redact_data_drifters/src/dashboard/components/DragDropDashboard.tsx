...
export function DragDropDashboard({ comparisonMode = false }: DragDropDashboardProps) {
  ...
  const duplicateWidget = (widgetId: string) => {
    ...
    // Add to layout
    const currentLayout = layouts.lg?.find((l: Layout) => l.i === widgetId);
    if (currentLayout) {
  ...
  const exportWidget = async (widgetId: string) => {
