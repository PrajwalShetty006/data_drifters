...
export function CustomerSegmentChart({ onExpand }: CustomerSegmentChartProps) {
  ...
  const { transformedSegments, loading, error } = useData();
  
  // Apply filters to customer segments
  const filteredSegments = useMemo(() => {
    if (!filters.metrics.segments) return [];
    if (!transformedSegments || transformedSegments.length === 0) return [];
    return applyFilters(transformedSegments, 'segment');
  }, [filters, applyFilters, transformedSegments]);
    ...
    >
      <Card className="border-blue-500/20 bg-gradient-to-br from-blue-950/70 to-blue-900/40 backdrop-blur-sm h-full">
        <div className="p-4 sm:p-6">
          ...
          {loading.rfm || loading.runAll ? (
            <div className="flex items-center justify-center h-[400px] text-blue-300">
              <p>Loading segment data...</p>
            </div>
          ) : error.runAll || error.rfm ? (
            <div className="flex flex-col items-center justify-center h-[400px] text-yellow-400 p-4 text-center">
