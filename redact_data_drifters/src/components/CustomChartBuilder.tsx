...
import { salesForecastData, customerSegments, rfmData, targetedOffers } from '../data/mockData';
import { 
...
export function CustomChartBuilder({ isOpen, onClose }: CustomChartBuilderProps) {
  ...
  const rawData = useMemo(() => {
    if (dataSource === 'forecast') return salesForecastData;
    if (dataSource === 'segments') return customerSegments;
    if (dataSource === 'rfm') return rfmData.slice(0, 50); // Limit RFM data
    if (dataSource === 'offers') return targetedOffers;
